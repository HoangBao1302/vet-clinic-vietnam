...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.id) {
      const existing = await TradingAccount.findOne({ id: body.id });
      if (existing) {
        return NextResponse.json(
          { error: "Trading account ID already exists" },
          { status: 400 }
        );
      }
    }
    ...
    return NextResponse.json(
      { error: "Failed to create trading account", message: error.message },
      { status: 500 }
    );
  }
}
