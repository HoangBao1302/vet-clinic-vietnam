...
export default function BlogPage() {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    if (!newsletterEmail.trim()) {
      setNewsletterMessage(t('blogPage.newsletter.emailPlaceholder'));
      return;
    }
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          topic: "support",
          message: locale === 'vi' 
            ? `Đăng ký nhận bài viết mới từ blog EA ThebenchmarkTrader.\n\nEmail: ${newsletterEmail}`
            : `Subscribe to receive new articles from EA ThebenchmarkTrader blog.\n\nEmail: ${newsletterEmail}`,
        }),
      });
      ...
      if (result.ok) {
        setNewsletterMessage(t('blogPage.newsletter.successMessage'));
        setNewsletterEmail("");
      } else {
        setNewsletterMessage(t('blogPage.newsletter.errorMessage') + " " + (result.error || ""));
      }
    } catch {
      setNewsletterMessage(t('blogPage.newsletter.connectionError'));
    } finally {
        ...
        <section className="py-3 md:py-12 bg-white border-b border-gray-200 sticky top-0 z-40">
          <div className="container-custom">
            <div className="flex flex-col lg:flex-row gap-2 md:gap-4 items-center justify-center">
              {['all', 'news', 'education', 'ea-leopard'].map((categoryId) => {
                const isActive = selectedCategory === categoryId;
                const count = getCategoryCount(categoryId);
                const categoryData = t(`blogPage.categories.${categoryId}`);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-center gap-2 md:gap-3">
                      <span className="text-lg md:text-2xl">{categoryData.name?.split(' ')[0] || '📚'}</span>
                      <div className="text-left">
                        <div className="font-bold text-sm md:text-lg">{categoryData.name}</div>
                        <div className={`text-xs md:text-sm ${isActive ? 'text-blue-100' : 'text-gray-500'}`}>
                          {count} {t('blogPage.postsCount')}
                        </div>
                      </div>
                    </div>
                  </button>
            ...
            <div className="mt-3 md:mt-6 text-center hidden md:block">
              <p className="text-gray-600 max-w-2xl mx-auto">
                {t(`blogPage.categories.${selectedCategory}.description`)}
              </p>
            </div>
          </div>
        </section>
                    ...
                    <div className="relative h-64 md:h-80">
                      ...
                      <div className="absolute top-4 left-4">
                        <span className="bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-medium">
                          {t(`blogPage.categories.${featuredPost.category}.name`)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-8">
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-4">
                          ...
                          <div className="flex items-center space-x-1">
                            <Calendar size={16} />
                            <span>{new Date(featuredPost.publishedAt || featuredPost.createdAt).toLocaleDateString(locale === 'vi' ? 'vi-VN' : 'en-US')}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            <Clock size={16} />
                          ...
                          <div className="flex items-center space-x-1">
                            <Eye size={16} />
                            <span>{featuredPost.views.toLocaleString()} {t('blogPage.views')}</span>
                          </div>
          ...
          <div className="container-custom">
            {loading ? (
              <div className="text-center py-20">
                ...
                <p className="text-gray-600">{t('blogPage.loading')}</p>
              </div>
            ) : regularPosts.length > 0 ? (
              <>
                <div className="text-center mb-6 md:mb-12">
                  <h2 className="text-xl md:text-3xl font-bold text-gray-800 mb-2 md:mb-4">
                    {selectedCategory === "all" ? t('blogPage.allPosts') : t('blogPage.otherPosts')}
                  </h2>
                  <p className="text-sm md:text-lg text-gray-600">
                    {regularPosts.length} {t('blogPage.postsInCategory')}
                  </p>
                </div>
                        ...
                        <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer h-full hover:translate-y-[-4px]">
                          <div className="relative h-48">
                            ...
                            <div className="absolute top-3 left-3">
                              <span className="bg-white text-gray-800 px-2 py-1 rounded-full text-xs font-medium">
                                {t(`blogPage.categories.${post.category}.name`)}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500">
                            <div className="flex items-center space-x-3">
                              <div className="flex items-center space-x-1">
                                <Calendar size={12} />
                                <span>{new Date(post.publishedAt || post.createdAt).toLocaleDateString(locale === 'vi' ? 'vi-VN' : 'en-US')}</span>
                              </div>
                              <div className="flex items-center space-x-1">
                                <Clock size={12} />
              ...
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold text-gray-800 mb-2">
                  {t('blogPage.noPosts.title')}
                </h3>
                <p className="text-gray-600">
                  {t('blogPage.noPosts.subtitle')}
                </p>
              </div>
            )}
          </div>
