...
export default function DownloadsPage() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      if (!token && !user && isAuthenticated === false) {
        ...
        router.push(`/${locale}/login?redirect=/${locale}/downloads`);
      }
    };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mt-2">{t('checkingAccessHint')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!isAuthenticated && !hasAuthData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">{t('redirectingLogin')}</p>
        </div>
      </div>
    );
  }
  ...
  const handleVerifyOrder = async (itemId: string) => {
      if (!orderCode.trim()) {
      setVerifyMessage(t('paidItems.verifyOrder.required'));
      return;
    }
    ...
    try {
      ...
      if (result.verified) {
        setVerifyMessage(t('paidItems.verifyOrder.success'));
        ...
        let errorMsg = result.error || t('paidItems.verifyOrder.error');
        ...
        if (result.actualProductId && result.requestedProductId) {
          ...
          if (actualProduct) {
            errorMsg += ' ' + t('paidItems.verifyOrder.productMismatch', { product: actualProduct.name });
          }
        }
      ...
      setVerifyMessage("❌ " + t('paidItems.verifyOrder.connectionError'));
    } finally {
      setTimeout(() => {
          ...
          <div className="container-custom">
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-800 rounded-full mb-4">
                <FileText size={20} />
                <span className="font-semibold">{t('pdfGuides.badge')}</span>
              </div>
              <h2 className="text-3xl font-bold text-gray-800 mb-4">
                {t('pdfGuides.title')}
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                {t('pdfGuides.description')}
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {pdfGuides.map((item) => (
                <div key={item.id} className="bg-white border-2 border-gray-200 rounded-xl p-6 hover:border-blue-500 hover:shadow-xl transition-all">
                  ...
                  >
                    <Download size={18} />
                    <span>{t('pdfGuides.download')}</span>
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="container-custom">
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-800 rounded-full mb-4">
                <Gift size={20} />
                <span className="font-semibold">{t('freeItems.badge')}</span>
              </div>
              <h2 className="text-3xl font-bold text-gray-800 mb-4">
                {t('freeItems.title')}
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                {t('freeItems.description')}
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {freeItems.map((item) => (
                <div key={item.id} className="bg-white border-2 border-gray-200 rounded-xl p-6 hover:border-green-500 hover:shadow-xl transition-all">
                  ...
                  >
                    <Download size={18} />
                    <span>{t('freeItems.download')}</span>
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="container-custom">
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-purple-100 text-purple-800 rounded-full mb-4">
                <ShieldCheck size={20} />
                <span className="font-semibold">{t('paidItems.badge')}</span>
              </div>
              <h2 className="text-3xl font-bold text-gray-800 mb-4">
                {t('paidItems.title')}
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-6">
                {t('paidItems.description')}
              </p>
              <div className="flex items-center justify-center gap-4 text-sm text-gray-600">
                <span className="flex items-center gap-2">
                  <CreditCard size={16} />
                  {t('paidItems.paymentMethods')}
                </span>
              </div>
            </div>
                ...
                {paidItems.filter(item => item.platform === "MT4").map((item) => (
                  <div key={item.id} className="bg-white border-2 border-purple-200 rounded-xl p-6 hover:border-purple-500 hover:shadow-2xl transition-all">
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      >
                        <CreditCard size={18} />
                        <span>{t('paidItems.buyWithStripe')}</span>
                      </button>
                      <button
                      ...
                      >
                        <CreditCard size={18} />
                        <span>{t('paidItems.buyWithPayPal')}</span>
                      </button>
                    </div>
                    ...
                    <div className="pt-4 border-t border-gray-200">
                      <p className="text-xs text-gray-600 mb-2 text-center">
                        {t('paidItems.verifyOrder.description')}
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
                          placeholder={t('paidItems.verifyOrder.orderCode')}
                        ...
                        >
                          {verifyingOrder === item.id ? t('paidItems.verifyOrder.verifying') : t('paidItems.verifyOrder.verify')}
                        </button>
                      </div>
